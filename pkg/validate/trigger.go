...
func (t *Trigger) validate(triggerWhen []buildapi.TriggerWhen) []error {
	...
	for _, when := range triggerWhen {
		if when.Name == "" {
			t.build.Status.Reason = buildapi.BuildReasonPtr(buildapi.TriggerNameCanNotBeBlank)
		...
		case buildapi.GitHubWebHookTrigger:
			if when.GitHub == nil {
				t.build.Status.Reason = buildapi.BuildReasonPtr(buildapi.TriggerInvalidGitHubWebHook)
		...
		case buildapi.ImageTrigger:
			if when.Image == nil {
				t.build.Status.Reason = buildapi.BuildReasonPtr(buildapi.TriggerInvalidImage)
				t.build.Status.Message = pointer.String(fmt.Sprintf(
		...
		case buildapi.PipelineTrigger:
			if when.ObjectRef == nil {
				t.build.Status.Reason = buildapi.BuildReasonPtr(buildapi.TriggerInvalidPipeline)
				t.build.Status.Message = pointer.String(fmt.Sprintf(
		...
		default:
			t.build.Status.Reason = buildapi.BuildReasonPtr(buildapi.TriggerInvalidType)

...
func (src *BuildRun) ConvertTo(ctx context.Context, obj *unstructured.Unstructured) error {
	...
	// Handle conversion of ImageBuildrun to TaskRunName for backward compatibility
	var taskRunName *string
	if src.Status.ImageBuildrun != nil {
		// If ImageBuildrun is set, use its Name field
		taskRunName = &src.Status.ImageBuildrun.Name
	} else {
...
func (src *BuildRun) ConvertFrom(ctx context.Context, obj *unstructured.Unstructured) error {
	...
	// Handle conversion from v1alpha1 LatestTaskRunRef to v1beta1 ImageBuildrun
	var imageBuildrun *ImageBuildrun
	if alphaBuildRun.Status.LatestTaskRunRef != nil {
		// Convert the old TaskRunName to new ImageBuildrun structure
		// Since v1alpha1 only had TaskRun support, we default to "TaskRun" kind
		imageBuildrun = &ImageBuildrun{
	...
	src.Status = BuildRunStatus{
		...
		Output:         output,
		Conditions:     conditions,
		TaskRunName:    alphaBuildRun.Status.LatestTaskRunRef, // Keep for backward compatibility
		ImageBuildrun:  imageBuildrun,                         // New field with enhanced information

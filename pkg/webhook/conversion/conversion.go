...
type convertFunc func(ctx context.Context, Object *unstructured.Unstructured, version string) (*unstructured.Unstructured, metav1.Status)
...
func CRDConvert(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	serve(ctx, w, r, convertSHPCR)
}
...
func serve(ctx context.Context, w http.ResponseWriter, r *http.Request, convert convertFunc) {
	var body []byte
	if r.Body != nil {
	...
	if err != nil {
		ctxlog.Error(ctx, err, "outserializer encoding failed")
...
func doConversion(ctx context.Context, convertRequest *v1.ConversionRequest, convert convertFunc) *v1.ConversionResponse {
	...
	for _, obj := range convertRequest.Objects {
		...
		convertedCR, status := convert(ctx, &cr, convertRequest.DesiredAPIVersion)

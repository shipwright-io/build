...
func (r *ReconcileBuildRun) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if buildRunnerErr != nil {
		if apierrors.IsNotFound(buildRunnerErr) {
			...
			// Check volumes exist using the TaskRuns from the interface
			if err := r.validateTaskRunVolumes(ctx, imageBuildRunner); err != nil {
				...
				if apierrors.IsNotFound(err) {
					...
					if updateErr := resources.UpdateConditionWithFalseStatus(ctx, r.client, buildRun, err.Error(), string(buildv1beta1.VolumeDoesNotExist)); updateErr != nil {
						return reconcile.Result{}, updateErr
					}
...
// validateTaskRunVolumes checks that all volumes referenced in the TaskRuns exist.
func (r *ReconcileBuildRun) validateTaskRunVolumes(ctx context.Context, imageBuildRunner ImageBuildRunner) error {
	...
	// For PipelineRuns, if no TaskRuns exist yet (ChildReferences empty), skip volume validation
	// as the PipelineRun hasn't been processed by Tekton yet

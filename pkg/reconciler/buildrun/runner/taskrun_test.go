...
var _ = Describe("TaskRun SyncBuildRunStatus", func() {
	...
	When("the build has started", func() {
		...
		Context("and the TaskRun is in progress", func() {
			...
			PIt("updates Succeded status to Unknown if the TaskRun has started")

			It("updates Succeded status to Unknown if the TaskRun is running", func(ctx SpecContext) {
				succeededCondition.Reason = pipelinev1.TaskRunReasonRunning.String()
				runner.TaskRun.Status.SetCondition(succeededCondition)

				Expect(runner.SyncBuildRunStatus(ctx, k8sClient, buildRun)).To(Succeed(), "sync BuildRun status")
				buildRunCondition := buildRun.Status.GetCondition(v1beta1.Succeeded)
				Expect(buildRunCondition).NotTo(BeNil(), "succeded condition")
		...
		Context("and the TaskRun has timed out", func() {
			...
			It("updates Succeded status to False if the TaskRun has timed out", func(ctx SpecContext) {
				Expect(runner.SyncBuildRunStatus(ctx, k8sClient, buildRun)).To(Succeed(), "sync BuildRun status")
		...
		Context("and the TaskRun has completed with a failure", func() {
			...
			It("updates Succeded status to False if the TaskRun has failed due to pod eviction", func(ctx SpecContext) {
			...
			It("updates Succeded status to False if the TaskRun has failed due to to OOMKilled", func(ctx SpecContext) {
			...
			It("updates Succeded status to False if the TaskRun has failed due to the build failing", func(ctx SpecContext) {
			...
			It("updates Succeded status to False if the TaskRun has failed to a vulnerability scan", func(ctx SpecContext) {
		...
		Context("and the TaskRun has completed with success", func() {
			...
			PIt("updates Succeded status to True if the TaskRun has succeeded")
	...
	When("the build has been cancelled", func() {
		...
		PIt("updates Succeded status to Unknown if the TaskRun has not been canceled yet")

		PIt("updates Succeded status to False if the TaskRun has been canceled")
	})
})

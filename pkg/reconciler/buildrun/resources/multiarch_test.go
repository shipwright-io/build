...
var _ = Describe("Multi-Arch PipelineRun Generation", func() {
	...
	Context("when spec.output.platforms is configured on the Build", func() {
		BeforeEach(func() {
			build.Spec.Output.Platforms = []buildapi.ImagePlatform{
				{OS: "linux", Arch: "amd64"},
				{OS: "linux", Arch: "arm64"},
			}
		})

		It("generates a PipelineRun with per-platform build tasks and correct ordering", func() {
		...
		It("generates TaskRunSpecs with per-platform nodeSelector matching spec.output.platforms (scheduling contract)", func() {
			pr, err := resources.GeneratePipelineRun(cfg, build, buildRun, serviceAccountName, clusterBuildStrategy)
			Expect(err).ToNot(HaveOccurred())

			Expect(pr.Spec.TaskRunSpecs).To(HaveLen(2))

			want := map[string]struct{ os, arch string }{
				"build-linux-amd64": {os: "linux", arch: "amd64"},
				"build-linux-arm64": {os: "linux", arch: "arm64"},
			}
			for _, s := range pr.Spec.TaskRunSpecs {
				Expect(s.PodTemplate).ToNot(BeNil())
				ns := s.PodTemplate.NodeSelector
				w, ok := want[s.PipelineTaskName]
				Expect(ok).To(BeTrue(), "unexpected task %s", s.PipelineTaskName)
				Expect(ns[corev1.LabelOSStable]).To(Equal(w.os))
				Expect(ns[corev1.LabelArchStable]).To(Equal(w.arch))
			}
		})

		It("merges non-os/arch nodeSelector from Build into per-platform TaskRunSpecs", func() {
			...
			for _, s := range pr.Spec.TaskRunSpecs {
				Expect(s.PodTemplate.NodeSelector["disktype"]).To(Equal("ssd"))
				Expect(s.PodTemplate.NodeSelector[corev1.LabelOSStable]).To(Equal("linux"))
			}
		})

		It("propagates ClusterBuildStrategy parameters to each per-platform pipeline task (strategy contract)", func() {
			pr, err := resources.GeneratePipelineRun(cfg, build, buildRun, serviceAccountName, clusterBuildStrategy)
			Expect(err).ToNot(HaveOccurred())
			...
			for _, task := range pr.Spec.PipelineSpec.Tasks {
				...
				for _, p := range task.Params {
					if p.Name == "exit-command" {
						found = true
						Expect(p.Value.StringVal).To(Equal("$(params.exit-command)"))
					}
				}
				Expect(found).To(BeTrue(), "task %s should reference strategy parameter exit-command", task.Name)
			}
		})
		...
		It("overrides shp-output-image and includes image-processing step in per-platform tasks", func() {
			pr, err := resources.GeneratePipelineRun(cfg, build, buildRun, serviceAccountName, clusterBuildStrategy)
			Expect(err).ToNot(HaveOccurred())
			...
			for _, task := range pr.Spec.PipelineSpec.Tasks {
				...
				expectedSuffix := "-linux-amd64"
				if task.Name == "build-linux-arm64" {
					expectedSuffix = "-linux-arm64"
				}
				for _, p := range task.Params {
					if p.Name == "shp-output-image" {
						Expect(p.Value.StringVal).To(ContainSubstring(expectedSuffix))
					}
				}
		...
		Context("when the strategy defines a SecurityContext", func() {
			BeforeEach(func() {
				clusterBuildStrategy.Spec.SecurityContext = &buildapi.BuildStrategySecurityContext{
					RunAsUser:  1000,
					RunAsGroup: 1000,
				}
			})
		...
		It("wires SourceTimestamp from source-acquisition to per-platform tasks via pipeline parameter", func() {
			sourceTimestamp := buildapi.OutputImageSourceTimestamp
		...
		It("includes vulnerability scanning args in per-platform image-processing when enabled", func() {
			build.Spec.Output.VulnerabilityScan = &buildapi.VulnerabilityScanOptions{
				Enabled: true,
			}
		...
		Context("when the strategy references output-directory", func() {
			...
			It("adds output-directory param to pipeline spec and --push to per-platform image-processing", func() {
				pr, err := resources.GeneratePipelineRun(cfg, build, buildRun, serviceAccountName, clusterBuildStrategy)
				Expect(err).ToNot(HaveOccurred())

...
var _ = Describe("GenerateTaskrun", func() {
	var (
		...
		buildWithEnvs          *buildapi.Build
		buildRun               *buildapi.BuildRun
		buildRunWithEnvs       *buildapi.BuildRun
		buildStrategy          *buildapi.BuildStrategy
		buildStrategyStepNames map[string]struct{}
		buildStrategyWithEnvs  *buildapi.BuildStrategy
		buildpacks             string
		ctl                    test.Catalog
	)
	...
	Describe("Generate the TaskSpec", func() {
		...
		Context("when env vars are defined", func() {
			...
			It("should contain env vars specified in Build in every BuildStrategy step", func() {
				got, err = resources.GenerateTaskSpec(config.NewDefaultConfig(), buildWithEnvs, buildRun, buildStrategy.Spec.Steps, []buildapi.Parameter{}, buildStrategy.GetVolumes())
				Expect(err).To(BeNil())
			...
			It("should contain env vars specified in BuildRun in every step", func() {
				got, err = resources.GenerateTaskSpec(config.NewDefaultConfig(), build, buildRunWithEnvs, buildStrategy.Spec.Steps, []buildapi.Parameter{}, buildStrategy.GetVolumes())
				Expect(err).To(BeNil())
			...
			It("should override Build env vars with BuildRun env vars in every step", func() {
				got, err = resources.GenerateTaskSpec(config.NewDefaultConfig(), buildWithEnvs, buildRunWithEnvs, buildStrategy.Spec.Steps, []buildapi.Parameter{}, buildStrategy.GetVolumes())
				Expect(err).To(BeNil())
			...
			It("should fail attempting to override an env var in a BuildStrategy", func() {
				got, err = resources.GenerateTaskSpec(config.NewDefaultConfig(), buildWithEnvs, buildRunWithEnvs, buildStrategyWithEnvs.Spec.Steps, []buildapi.Parameter{}, buildStrategy.GetVolumes())
		...
		Context("when only BuildRun has output image labels and annotation defined ", func() {
			BeforeEach(func() {
				...
				JustBeforeEach(func() {
					got, err = resources.GenerateTaskSpec(config.NewDefaultConfig(), build, buildRun, buildStrategy.Spec.Steps, []buildapi.Parameter{}, buildStrategy.GetVolumes())
					Expect(err).To(BeNil())
				})
	...
	Describe("Generate the TaskRun", func() {
		...
		Context("when the taskrun is generated by default", func() {
			...
			It("should ensure generated TaskRun's basic information are correct", func() {
				...
				Expect(got.Labels[buildapi.LabelBuild]).To(Equal(build.Name))
				Expect(got.Labels[buildapi.LabelBuildRun]).To(Equal(buildRun.Name))
				Expect(got.Labels[buildapi.LabelBuildStrategyName]).To(Equal(build.Spec.Strategy.Name))
				Expect(got.Labels[buildapi.LabelBuildStrategyGeneration]).To(Equal("0"))
			})

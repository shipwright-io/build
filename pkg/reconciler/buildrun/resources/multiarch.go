...
func generateSourceBundlePushStep(cfg *config.Config, taskSpec *pipelineapi.TaskSpec, pushSecret *string) pipelineapi.Step {
	args := []string{
		...
		"--result-file-image-digest", fmt.Sprintf("$(results.%s-%s.path)", prefixParamsResultsVolumes, sourceBundleDigestResult),
	}
...
func createPerPlatformBuildTask(
	...
	// Pull source bundle by digest to avoid race conditions and tag movements.
	// When source is OCI artifact: use digest from source-acquisition appended to original ref
	// When source is bundled: use digest from source-bundle push appended to bundle ref
	var sourceBundleImage string
	if build.Spec.Source != nil && build.Spec.Source.Type == buildapi.OCIArtifactType && build.Spec.Source.OCIArtifact != nil {
		// Append digest to original OCI artifact ref to handle prune and tag movements
		// This gives us: registry/repo:tag@sha256:digest (digest takes precedence)
		sourceBundleImage = fmt.Sprintf("%s@$(tasks.source-acquisition.results.%s)",
			build.Spec.Source.OCIArtifact.Image,
			sources.TaskResultName(defaultSourceName, "image-digest"))
	} else {
		// Use digest from source bundle push
		sourceBundleImage = fmt.Sprintf("%s@$(tasks.source-acquisition.results.%s-%s)",
			sourceBundleImageParam(),
			prefixParamsResultsVolumes,
			sourceBundleDigestResult)
	}

...
// Steps share the same Pod filesystem, enabling efficient data transfer between phases.
...
func (g *TaskRunGenerator) GenerateSourceAcquisitionPhase(execCtx *executionContext) error {
...
func (g *TaskRunGenerator) GenerateOutputImagePhase(execCtx *executionContext) error {
...
func (g *TaskRunGenerator) ApplyInfrastructureConfiguration() error {
	if err := applyNodeSelectors(g.taskRun, g.build, g.buildRun); err != nil {
		return fmt.Errorf("failed to apply node selectors: %w", err)
	}

	if err := applyTolerations(g.taskRun, g.build, g.buildRun); err != nil {
		return fmt.Errorf("failed to apply tolerations: %w", err)
	}

	if err := applyScheduler(g.taskRun, g.build, g.buildRun); err != nil {
		return fmt.Errorf("failed to apply scheduler configuration: %w", err)
	}
...
func (g *TaskRunGenerator) ApplyMetadataConfiguration() error {
	if err := applyAnnotationsAndLabels(g.taskRun, g.strategy); err != nil {
		return fmt.Errorf("failed to apply annotations and labels: %w", err)
	}

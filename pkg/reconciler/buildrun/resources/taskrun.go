...
func buildTaskSpec(cfg *config.Config, build *buildv1beta1.Build, buildRun *buildv1beta1.BuildRun, strategy buildv1beta1.BuilderStrategy) (*pipelineapi.TaskSpec, error) {
	...
	applyStrategyParametersToTaskSpec(taskSpec, strategy.GetParameters())
	...
	if err := applyBuildStrategySteps(taskSpec, build, strategy.GetBuildSteps(), combinedEnvs); err != nil {
		return nil, fmt.Errorf("failed to apply build strategy steps for strategy %q: %w", strategy.GetName(), err)
	}

	volumeMounts, err := validateAndCollectVolumeMounts(strategy.GetBuildSteps(), strategy.GetVolumes())
	if err != nil {
		return nil, fmt.Errorf("failed to validate and collect volume mounts for strategy %q: %w", strategy.GetName(), err)
	}
...
func applyBuildStrategySteps(
	...
	for _, containerValue := range buildSteps {
		...
		if err != nil {
			return fmt.Errorf("error(s) occurred merging environment variables into BuildStrategy %q steps: %s", build.Spec.StrategyName(), err.Error())
		}
...
// validateAndCollectVolumeMounts validates that all volume mounts reference valid volumes and collects them
func validateAndCollectVolumeMounts(
	buildSteps []buildv1beta1.Step,
...
// applyStrategyParametersToTaskSpec applies strategy-defined parameters to TaskSpec
func applyStrategyParametersToTaskSpec(taskSpec *pipelineapi.TaskSpec, parameterDefinitions []buildv1beta1.Parameter) {

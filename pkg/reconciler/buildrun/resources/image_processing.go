...
func BuildImageProcessingArgs(
	...
	// Check if we need to set image annotations
	...
	// Add common arguments if anything was added
	if len(stepArgs) > 0 {
...
// CreateImageProcessingStep creates the image-processing step and adds it to the TaskSpec.
// This step handles image mutation (labels, annotations) and pushing to the registry.
func CreateImageProcessingStep(
	...
	// Add output directory volume mount if needed
	if hasOutputDirectory {
		prefixedOutputDirectory := fmt.Sprintf("%s-%s", prefixParamsResultsVolumes, paramOutputDirectory)
	...
	// Append the step to the TaskSpec
...
// This is a convenience function that combines all three image processing operations
// for TaskRun execution. For PipelineRun, use the individual functions separately.
func SetupImageProcessing(taskRun *pipelineapi.TaskRun, cfg *config.Config, creationTimestamp time.Time, buildOutput, buildRunOutput build.Image) error {
	// Setup output directory (scans steps and adds parameter/volume if needed)
	// Convert Params to []Param for SetupOutputDirectory

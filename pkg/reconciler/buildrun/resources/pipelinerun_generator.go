...
func (g *PipelineRunGenerator) GenerateSourceAcquisitionPhase(_ *executionContext) error {
	...
	if g.isMultiArch() && !g.isSourceOCIArtifact() {
		...
		// Add result for source bundle digest so platform tasks can pull by digest
		taskSpec.Results = append(taskSpec.Results, pipelineapi.TaskResult{
			Name:        fmt.Sprintf("%s-%s", prefixParamsResultsVolumes, sourceBundleDigestResult),
			Description: "The digest of the source bundle OCI artifact",
		})
	}

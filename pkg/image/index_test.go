...
var _ = Describe("AssembleImageIndex", func() {
	...
	It("assembles an image index from two platform images", func() {
		...
		// nil options are intentional: go-containerregistry treats plain-HTTP
		// localhost registries as insecure by default when no transport is
		// configured, so no remote.WithTransport(insecure) is needed here.
	...
	It("returns an error when a platform image does not exist", func() {
		...
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("getting descriptor"))
	})
})

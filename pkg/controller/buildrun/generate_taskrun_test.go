...
var _ = Describe("GenerateTaskrun", func() {
	...
	Describe("Generate the TaskSpec", func() {
		...
		Context("when the task spec is generated", func() {
			BeforeEach(func() {
				build, err = t.LoadBuildYAML([]byte(test.MinimalBuildahBuild))
				Expect(err).To(BeNil())

				buildRun, err = t.LoadBuildRunYAML([]byte(test.MinimalBuildahBuildRun))
				Expect(err).To(BeNil())

				buildStrategy, err = t.LoadBuildStrategyYAML([]byte(test.MinimalBuildahBuildStrategy))
				Expect(err).To(BeNil())
			...
			It("should ensure resource replacements happen for the first step", func() {
				Expect(got.Steps[0].Container.Resources).To(Equal(t.LoadCustomResources("500m", "1Gi")))
			})

			It("should ensure resource replacements happen for the second step", func() {
				Expect(got.Steps[1].Container.Resources).To(Equal(t.LoadCustomResources("100m", "65Mi")))
			})
	...
	Describe("Generate the TaskRun", func() {
		...
		Context("when the taskrun is generated by default", func() {
			BeforeEach(func() {
				build, err = t.LoadBuildYAML([]byte(test.BuildahBuildWithOutput))
				Expect(err).To(BeNil())

				buildRun, err = t.LoadBuildRunYAML([]byte(test.BuildahBuildRunWithSA))
				Expect(err).To(BeNil())

				buildStrategy, err = t.LoadBuildStrategyYAML([]byte(test.BuildahBuildStrategySingleStep))
				Expect(err).To(BeNil())
		...
		Context("when the taskrun is generated by special settings", func() {
			BeforeEach(func() {
				build, err = t.LoadBuildYAML([]byte(test.BuildpacksBuildWithBuilderAndTimeOut))
				Expect(err).To(BeNil())

				buildRun, err = t.LoadBuildRunYAML([]byte(test.BuildpacksBuildRunWithSA))
				Expect(err).To(BeNil())

				buildStrategy, err = t.LoadBuildStrategyYAML([]byte(test.BuildpacksBuildStrategySingleStep))
				Expect(err).To(BeNil())
			})
		...
		Context("when the build and buildrun contain a timeout", func() {
			BeforeEach(func() {
				build, err = t.LoadBuildYAML([]byte(test.BuildahBuildWithTimeOut))
				Expect(err).To(BeNil())

				buildRun, err = t.LoadBuildRunYAML([]byte(test.BuildahBuildRunWithTimeOutAndSA))
				Expect(err).To(BeNil())
		...
		Context("when the build and buildrun both contain an output imageURL", func() {
			BeforeEach(func() {
				...
				buildRun, err = t.LoadBuildRunYAML([]byte(test.BuildahBuildRunWithSAAndOutput))
				Expect(err).To(BeNil())

...
| `go-version`        | Version of Go, must match a tag from [the golang image](https://hub.docker.com/_/golang?tab=tags)                                                                                                                                                                                                  | `1.25`    |
...
spec:
  ...
  steps:
    ...
    - name: build-and-push
      image: moby/buildkit:v0.28.1-rootless
...
- We will use a modified buildah strategy, with the following steps resources:
  ...
  ```yaml
    - name: buildah-bud
      image: quay.io/containers/buildah:v1.43.0
      workingDir: $(params.shp-source-root)
      securityContext:
        privileged: true
      command:

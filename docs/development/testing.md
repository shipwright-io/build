...
| `TEST_E2E_CONTROLLER`             | `start_local`                                                                                    | String with allowed values `start_local` (will start the controller locally and print its logs add the end of the test run) and `managed_outside` (will assume that the controller is running through whatever means) |

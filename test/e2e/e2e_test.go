...
var _ = Describe("For a Kubernetes cluster with Tekton and build installed", func() {
	...
	Context("when a build definition with invalid timeout value", func() {

		BeforeEach(func() {
			testID = generateTestID("kaniko-invalidTimeout")

			// create the build definition
			createBuild(namespace, testID, "test/data/build_invalidtimeout.yaml")
		})
		...
		It("failed a build", func() {
			validateBuildToFail(namespace, b, "invalid timout duration")
		})
	})

	...
	Context("when LocalCopy BuildSource is defined", Label("LocalSource", "FailureCase"), func() {
		BeforeEach(func() {
			...
			build = createBuild(
				testBuild,
				testID,
				"test/data/v1alpha1/build_buildah_cr_local_source_upload.yaml",
			)
		})
		...
		It("should generate LocalCopy TaskRun, using the waiter", func() {
			...
			buildRun, err = buildRunTestData(
				testBuild.Namespace,
				testID,
				"test/data/v1alpha1/buildrun_buildah_cr_local_source_upload.yaml",
			)
...
func getBuildRunStatusCondition(name types.NamespacedName) *buildapialpha.Condition {
	...
	return testBuildRun.Status.GetCondition(buildapialpha.Succeeded)
}
...
func validateWaiterBuildRun(testBuild *utils.TestBuild, testBuildRun *buildapialpha.BuildRun) {

...
var _ = Describe("For a Kubernetes cluster with Tekton and build installed", func() {
	...
	Context("when a Buildah build with label and annotation is defined", Label("FEATURE:ImageMutation", "FEATURE:Buildah", "CORE"), func() {
		BeforeEach(func() {
	...
	Context("mutate image timestamp", Label("FEATURE:ImageMutation", "CORE"), func() {
		...
		Context("when using BuildKit based Dockerfile build", Label("FEATURE:BuildKit"), func() {
		...
		Context("when using Buildpacks build", Label("FEATURE:Buildpacks"), func() {
			var sampleBuildRun = func(outputTimestamp string) *buildv1alpha1.BuildRun {
				return NewBuildRunPrototype().
					Namespace(testBuild.Namespace).
					Name(testID).
		...
		Context("edge cases", Label("FAILING"), func() {

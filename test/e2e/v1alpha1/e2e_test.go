	...
	Context("when a Kaniko+Trivy build is defined to use an image with no critical CVEs", func() {
		...
		BeforeEach(func() {
			...
			build = createBuild(
				testBuild,
				testID,
				"samples/v1alpha1/build/build_kaniko-trivy-good_cr.yaml",
			)
		})

		It("successfully runs a build and surface results to BuildRun", func() {
			buildRun, err = buildRunTestData(testBuild.Namespace, testID, "samples/v1alpha1/buildrun/buildrun_kaniko-trivy-good_cr.yaml")
			Expect(err).ToNot(HaveOccurred())
	...
	Context("when a Kaniko+Trivy build is defined to use an image with a critical CVE", func() {
		...
		BeforeEach(func() {
			...
			build = createBuild(
				testBuild,
				testID,
				"samples/v1alpha1/build/build_kaniko-trivy-bad_cr.yaml",
			)
		})

		It("fails to run a build", func() {
			buildRun, err = buildRunTestData(testBuild.Namespace, testID, "samples/v1alpha1/buildrun/buildrun_kaniko-trivy-bad_cr.yaml")
			Expect(err).ToNot(HaveOccurred())

...
Previously, in https://github.com/shipwright-io/build/issues/97[issue #97], we've explored a different mechanism to receive users' data upload, instead of executing the regular `git clone`. This mechanism is comparable to `kubectl cp` and `oc rsync`.
